...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;对于订单的结算、收货、退货等操作，需要考虑并发的事务处理。在结算等操作之前开启事务，执行全部验证商品的库存和用户金额的操作，在验证完成并进行相应的处理后提交事务，如果事务出现问题则进行事务的回滚，恢复到未执行前的状态。
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;由创建的工程自动生成的数据库生成脚本，在附件中，存储在SQL文件夹下“DatabaseSQL.sql”文件中。执行该代码会创建数据库”Mail“和对应的18张表。而在执行Web项目样例之前还需要执行SQL文件夹下初始化脚本”init.sql“以便增加用户的商品等数据内容并创建视图等。
