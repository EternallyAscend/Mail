...
def Finish_Register(request):
    ...
    if request.POST:
        ...
        if not isPhone(telephone):
            return render(request,'register.html',{'Result':"Wrong telephone number."})
        name=request.POST['name']
        ...
        if pwd==password:
            ...
            id=""
            sql="select customer_id from customer where customer_name like %s and customer_register_time like %s;"
            cursor.execute(sql,[name,date])
            temp=cursor.fetchall()
            sql="insert into customer_telephone values(%s,%s);"
            cursor.execute(sql,[telephone,temp[0]['customer_id']])
            account="Your account ID is "+str(temp[0]['customer_id'])
            cursor.fetchall()
            database.commit()
            status=True
        else:
